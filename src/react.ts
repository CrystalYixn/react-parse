...
export function forwardRef<T>(funcComponent: (p: Props, ref: Ref<T>) => VDOM) {
  return {
...
function memo<T extends Props>(
  type: FunctionVDOMType<T>,
  compare: (prevProps: T, nextProps: T) => boolean = shallowEqual
) {
  return {
    $$typeof: REACT_MEMO,
    type,
